...
public class CarAgent : Agent
{
    ...
    private bool isPedestrianNearby = false;
    private bool isTrafficLightRed = false;

    private int waitingForPedestrianCounter = 0;
    private int waitingForTrafficLightCounter = 0;
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        // If a pedestrian is nearby, stop the car
        if (ShouldStopForPedestrian()) { 

            waitingForPedestrianCounter++;

            currentSpeed = currentSpeed / waitingForPedestrianCounter;
        }else {

            waitingForPedestrianCounter = 0;
        ...
        if (ShouldStopForRedLight())
        {
            ...
            waitingForTrafficLightCounter++;

            currentSpeed = currentSpeed / waitingForTrafficLightCounter;
        }
        else
        {

            waitingForTrafficLightCounter = 0;

        }


        rb.MovePosition(transform.position + transform.forward * moveForward * Time.fixedDeltaTime * currentSpeed);

        // Apply rotation only when there is movement along the Z-axis
        if (Mathf.Abs(moveForward) > 0.2f && currentSpeed > 0.2f)
        {
    ...
    public void SetPedestrianNearby(bool state)
    {
        isPedestrianNearby = state;
    }

    private bool ShouldStopForPedestrian()
    {
        return isPedestrianNearby;
    }

    public void SetTrafficLightRed(bool state)
    {
        isTrafficLightRed = state;
    }
